...
namespace Library
{
    class Utility final
    {
      public:
        ...
        static void LoadShaderFile( const std::wstring& filename,  const char* entryPoint, const char* target, std::vector<char>& data );
    ...
    template <typename T>
    void UpdateValue( std::function<bool()> increasePredicate, std::function<bool()> decreasePredicate, T& value,
                      const T& delta, std::function<void( const T& )> updateFunc = nullptr,
