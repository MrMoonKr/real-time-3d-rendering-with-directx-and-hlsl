...
namespace Library
{
    ...
    Grid::Grid( Game& game, const std::shared_ptr<Camera>& camera, std::uint32_t size, std::uint32_t scale,
                const XMFLOAT4& color )
        : DrawableGameComponent( game, camera ), mMaterial( *mGame ), mSize( size ), mScale( scale ), mColor( color )
    {
    }
    ...
    void Grid::InitializeGrid()
    {
        ...
        ThrowIfFailed( direct3DDevice->CreateBuffer( &vertexBufferDesc, &vertexSubResourceData, mVertexBuffer.put() ),
