...
namespace Library
{
	...
	void Skybox::Draw(const GameTime& gameTime)
	{
		...
		uint32_t stride = narrow_cast<uint32_t>(sizeof(VertexPositionTexture));
		uint32_t offset = 0;
	...
	void Skybox::CreateVertexBuffer(not_null<ID3D11Device*> device, const Mesh& mesh, not_null<ID3D11Buffer**> vertexBuffer) const
	{
		...
		vertexBufferDesc.ByteWidth = narrow_cast<uint32_t>(sizeof(VertexPositionTexture) * vertices.size());
