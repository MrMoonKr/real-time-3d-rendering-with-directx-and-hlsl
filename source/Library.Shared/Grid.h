...
namespace Library
{
    class Grid final : public DrawableGameComponent
    {
      ...
      public:
        Grid( Game& game, const std::shared_ptr<Camera>& camera, std::uint32_t size = DefaultSize,
              std::uint32_t scale = DefaultScale, const DirectX::XMFLOAT4& color = DefaultColor );
