...
namespace Library
{
    ...
    Game::Game( function<void*()> getWindowCallback, function<void( SIZE& )> getRenderTargetSizeCallback )
        : mGetWindow( getWindowCallback ), mGetRenderTargetSize( getRenderTargetSizeCallback ), mContentManager( *this )
    {
    ...
    /**
     * @brief ContentTypeReaderManager 초기화, GameClock 초기화, GameComponent들 초기화
     */
    ...
    void Game::Begin()
    {
        ...
        RenderTarget::Begin( Direct3DDeviceContext(), 
                             renderTargetViews,
                             not_null<ID3D11DepthStencilView*>( mDepthStencilView.get() ), 
                             mViewport );
    }
    ...
    void Game::CreateDeviceResources()
    {
        ...
        ThrowIfFailed( D3D11CreateDevice( nullptr, D3D_DRIVER_TYPE_HARDWARE, NULL, createDeviceFlags, featureLevels,
                                          narrow_cast<uint32_t>( size( featureLevels ) ), D3D11_SDK_VERSION,
                                          direct3DDevice.put(), &mFeatureLevel, direct3DDeviceContext.put() ),
        ...
        ThrowIfFailed( mDirect3DDevice->CheckMultisampleQualityLevels( DXGI_FORMAT_R8G8B8A8_UNORM, mMultiSamplingCount,
    ...
    void Game::CreateWindowSizeDependentResources()
    {
        ...
        if ( mSwapChain == nullptr )
        {
            ...
            ThrowIfFailed( dxgiFactory->CreateSwapChainForHwnd( mDirect3DDevice.get(), reinterpret_cast<HWND>( window ),
                                                                &swapChainDesc, &fullScreenDesc, nullptr,
                                                                mSwapChain.put() ),
                           "IDXGIDevice::CreateSwapChainForHwnd() failed." );
#else
            ThrowIfFailed( dxgiFactory->CreateSwapChainForCoreWindow( mDirect3DDevice.get(),
                                                                      reinterpret_cast<IUnknown*>( window ),
                                                                      &swapChainDesc, nullptr, mSwapChain.put() ),
                           "IDXGIFactory2::CreateSwapChainForCoreWindow() failed." );
#endif
        }
        else
        {
            // Resize existing swap chain
            HRESULT hr = mSwapChain->ResizeBuffers( DefaultBufferCount, mRenderTargetSize.cx, mRenderTargetSize.cy,
        ...
        ThrowIfFailed( mSwapChain->GetBuffer( 0, IID_PPV_ARGS( backBuffer.put() ) ),
                       "IDXGISwapChain::GetBuffer() failed." );
        backBuffer->GetDesc( &mBackBufferDesc );
        ThrowIfFailed( mDirect3DDevice->CreateRenderTargetView( backBuffer.get(), nullptr, mRenderTargetView.put() ),
                       "IDXGIDevice::CreateRenderTargetView() failed." );
        ...
        ThrowIfFailed( mDirect3DDevice->CreateTexture2D( &depthStencilDesc, nullptr, depthStencilBuffer.put() ),
                       "IDXGIDevice::CreateTexture2D() failed." );
...
#endif
        ThrowIfFailed( mDirect3DDevice->CreateDepthStencilView( depthStencilBuffer.get(), &depthStencilViewDesc,
                                                                mDepthStencilView.put() ),
                       "IDXGIDevice::CreateDepthStencilView() failed." );
