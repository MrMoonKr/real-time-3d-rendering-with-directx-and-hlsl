...
namespace Library
{
    ...
    void RenderTarget::Begin( not_null<ID3D11DeviceContext*> deviceContext,
                              const span<ID3D11RenderTargetView*>& renderTargetViews,
                              not_null<ID3D11DepthStencilView*> depthStencilView, const D3D11_VIEWPORT& viewport )
    {
        sRenderTargetStack.emplace( renderTargetViews, depthStencilView, viewport );
        deviceContext->OMSetRenderTargets( narrow_cast<uint32_t>( renderTargetViews.size() ), &renderTargetViews[ 0 ],
                                           depthStencilView );
    ...
    void RenderTarget::End( not_null<ID3D11DeviceContext*> deviceContext )
    {
        ...
        deviceContext->OMSetRenderTargets( renderTargetData.ViewCount(), renderTargetData.RenderTargetViews.data(),
                                           renderTargetData.DepthStencilView );
