...
namespace Library
{
    ...
    BasicMaterial::BasicMaterial( Game& game ) : Material( game )
    {
    }
    ...
    void BasicMaterial::Initialize()
    {
        ...
        auto& content = mGame->Content();
        auto vertexShader = content.Load<VertexShader>( L"Shaders\\BasicVS.hlsl" );
        SetShader( vertexShader );

        auto pixelShader = content.Load<PixelShader>( L"Shaders\\BasicPS.hlsl" );
        SetShader( pixelShader );
        ...
        ThrowIfFailed( direct3DDevice->CreateBuffer( &constantBufferDesc, nullptr, mVSConstantBuffer.put() ),
        ...
        ThrowIfFailed( direct3DDevice->CreateBuffer( &constantBufferDesc, nullptr, mPSConstantBuffer.put() ),
                       "ID3D11Device::CreateBuffer() failed." );
    ...
    {
        mGame->Direct3DDeviceContext()->UpdateSubresource( mVSConstantBuffer.get(), 0, nullptr,
                                                           worldViewProjectionMatrix.r, 0, 0 );
    }
