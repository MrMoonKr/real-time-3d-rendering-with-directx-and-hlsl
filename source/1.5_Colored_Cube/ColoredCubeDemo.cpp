...
namespace Rendering
{
	...
	void ColoredCubeDemo::Initialize()
	{
		...
		vertexBufferDesc.ByteWidth = narrow<uint32_t>(sizeof(VertexPositionColor) )* ARRAYSIZE(vertices);
		...
		indexBufferDesc.ByteWidth = narrow<uint16_t>(sizeof(uint16_t)) * mIndexCount;
	...
	void ColoredCubeDemo::Draw(const GameTime&)
	{
		...
		const uint32_t stride = narrow<uint32_t>(sizeof(VertexPositionColor));
