...
namespace Rendering
{
	...
	ProjectiveTextureMappingMaterial::ProjectiveTextureMappingMaterial(Game& game, shared_ptr<Texture2D> colorMap, shared_ptr<Texture2D> projectedMap, shared_ptr<Texture2D> depthMap) :
		Material(game),
	...
	{
		mDrawMode = drawMode;
		mPixelShaderClassInstance = mShaderClassInstances[DrawModeShaderClassMap.at(mDrawMode)];
	}

	com_ptr<ID3D11SamplerState> ProjectiveTextureMappingMaterial::SamplerState() const
	{
		return mSamplerState;
	}

	void ProjectiveTextureMappingMaterial::SetSamplerState(com_ptr<ID3D11SamplerState> samplerState)
	{
	...
	shared_ptr<Texture2D> ProjectiveTextureMappingMaterial::DepthMap() const
	{
		return mDepthMap;
	}

	void ProjectiveTextureMappingMaterial::SetDepthMap(shared_ptr<Texture2D> texture)
	{
		mDepthMap = move(texture);
	}
	...
	void ProjectiveTextureMappingMaterial::Initialize()
	{
		...
		mVertexShader = content.Load<VertexShader>(L"Shaders\\ProjectiveTextureMappingVS.cso"s);
		...
		mPixelShader = content.Load<PixelShader>(L"Shaders\\ProjectiveTextureMappingPS.cso", false, pixelShaderContentReader);
	...
	void ProjectiveTextureMappingMaterial::BeginDraw()
	{
		...
		ID3D11ShaderResourceView* const psShaderResources[] = { mColorMap->ShaderResourceView().get(), mProjectedMap->ShaderResourceView().get(), (mDepthMap != nullptr ? mDepthMap->ShaderResourceView().get() : nullptr) };
