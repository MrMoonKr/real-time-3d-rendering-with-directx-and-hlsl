...
int WINAPI WinMain( HINSTANCE instance, HINSTANCE, LPSTR, int showCommand )
{
    ...
    while ( message.message != WM_QUIT )
    {
        ...
        else
        {
            ...
            mDirect3DDeviceContext->ClearDepthStencilView( mDepthStencilView.get(),
...
void InitializeWindow( HINSTANCE instance, const wstring& className, const wstring& windowTitle, int showCommand )
{
    ...
    mWindowHandle = CreateWindow( className.c_str(), windowTitle.c_str(), WS_OVERLAPPEDWINDOW, center.x, center.y,
                                  windowRectangle.right - windowRectangle.left,
                                  windowRectangle.bottom - windowRectangle.top, nullptr, nullptr, instance, nullptr );
...
void InitializeDirectX()
{
    ...
    ThrowIfFailed( mDirect3DDevice->CheckMultisampleQualityLevels( DXGI_FORMAT_R8G8B8A8_UNORM, multiSamplingCount,
    ...
    ThrowIfFailed( dxgiFactory->CreateSwapChainForHwnd( mDirect3DDevice.get(), mWindowHandle, &swapChainDesc,
                                                        &fullScreenDesc, nullptr, mSwapChain.put() ),
                   "IDXGIDevice::CreateSwapChainForHwnd() failed." );

    com_ptr<ID3D11Texture2D> backBuffer;
    ThrowIfFailed( mSwapChain->GetBuffer( 0, IID_PPV_ARGS( backBuffer.put() ) ),
                   "IDXGISwapChain1::GetBuffer() failed." );
    ...
    ThrowIfFailed(
        mDirect3DDevice->CreateDepthStencilView( depthStencilBuffer.get(), nullptr, mDepthStencilView.put() ),
        "IDXGIDevice::CreateDepthStencilView() failed." );

    D3D11_VIEWPORT viewport = CD3D11_VIEWPORT( 0.0f, 0.0f, static_cast<float>( RenderTargetSize.cx ),
                                               static_cast<float>( RenderTargetSize.cy ) );
